...
def test_sampling_reproducibility_and_empirical_pmf():
    prior = PitmanYorCardinalityPrior(strength=1.0, discount=0.25)

    labels_a = prior.sample_partition(12, rng=7)
    labels_b = prior.sample_partition(12, rng=7)
    counts = prior.sample_number_of_clusters(5, num_samples=20, rng=3)

    assert labels_a == labels_b
    assert len(labels_a) == 12
    assert counts.shape == (20,)
    assert sum(empirical_pmf(counts)) == pytest.approx(1.0)


def test_empirical_pmf_respects_max_value():
    samples = np.asarray([0, 1, 1, 3])

    assert empirical_pmf(samples, max_value=4) == pytest.approx((0.25, 0.5, 0.0, 0.25, 0.0))
